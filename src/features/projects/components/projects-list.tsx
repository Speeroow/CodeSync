...
const formatTimestamp = (timstamp: number) => {
    return formatDistanceToNow(new Date(timstamp), { addSuffix: true })
}
...
const ProjectItem = ({ data }: { data: Doc<"projects"> }) => {
    return (
        <Link
            ...
            className="text-sm text-foreground/60 hover:text-foreground font-medium py-1 flex items-center justify-between w-full group"
        >
            <div className="flex items-center gap-2">
                {getProjectIcon(data)}
...
export const ProjectsList = ({ onViewAll }: ProjectsListProps) => {
    ...
    const [mostResent, ...rest] = projects;

    return (
        <div className="flex flex-col gap-4">
            {mostResent ? <ContinueCard data={mostResent} /> : null}
            {rest.length > 0 && (
                <div className="flex flex-col gap-2">
